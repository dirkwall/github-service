...
export class GitHubOperator implements SourceOperator {

  private static instance: GitHubOperator;

  private constructor() {
  }

  static getInstance() {
    if (GitHubOperator.instance === undefined) {
      GitHubOperator.instance = new GitHubOperator();
    }
    return GitHubOperator.instance;
  }

  async createRepository(gitHubOrgName : string, payload : CreateProjectRequest) : Promise<any> {
    const repository = {
  ...
  async setHook(gitHubOrgName : string, payload : CreateProjectRequest) : Promise<any> {
    try {
  ...
  async initialCommit(gitHubOrgName : string, payload : CreateProjectRequest) : Promise<any> {
    try {
      const repo = await gh.getRepo(gitHubOrgName, payload.data.application);
  ...
  async createBranchesForEachStages(gitHubOrgName : string, payload : CreateProjectRequest) : Promise<any> {
  ...
  async addShipyardToMaster(gitHubOrgName : string, payload : CreateProjectRequest) : Promise<any> {
    try {
      const repo = await gh.getRepo(gitHubOrgName, payload.data.application);
      await repo.writeFile('master',
                           'shipyard.yml',
